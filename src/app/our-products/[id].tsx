...
// ─── Bottle Illustration ──────────────────────────────────────────────────────
const BottleIllustration = React.memo(
  ({
    color,
    size,
    large,
        ...
        >
          <View
            style={{
              ...
              paddingHorizontal: large ? 10 : 4,
              paddingVertical: large ? 6 : 2,
              borderRadius: 3,
              marginBottom: 4,
            }}
          >
...
const Toast = React.memo(
  ({ message, visible }: { message: string; visible: boolean }) => {
...
export default function ProductDetail() {
  ...
  const loadProductData = useCallback(async () => {
    ...
    try {
      ...
      setRelatedProducts(
        allItems.filter((p: any) => String(p.id) !== String(id)),
      );
    } catch (err) {
        ...
        >
          <View
            style={[
              ...
              { backgroundColor: isDark ? C.darkCard : C.cream },
            ]}
          >
          ...
          <Text style={[styles.emptyDesc, { color: subColor }]}>
            This item may have been discontinued or moved in our real-time
            database catalog.
          </Text>
  ...
  return (
    <View style={[{ flex: 1, backgroundColor: bg }]}>
      ...
      <View
        style={[
          styles.header,
          { paddingTop: insets.top, backgroundColor: C.primaryBlue },
        ]}
      >
        <TouchableOpacity
          onPress={() => router.back()}
      ...
      >
        ...
        <Animated.View
          ...
          style={[styles.hero, { backgroundColor: C.primaryBlue }]}
        >
          <View style={styles.heroImageWrap}>
            <BottleIllustration
              color={product.imageColor || C.secondaryBlue}
              size={product.size}
              large
            />
          </View>
          ...
          <View style={{ paddingHorizontal: 20, paddingBottom: 8 }}>
            ...
            <Animated.View style={[cartAnimatedStyle, { marginTop: 24 }]}>
              <Pressable
                onPressIn={() => {
                  cartScale.value = withSpring(0.96);
                }}
                onPressOut={() => {
            ...
            <Animated.View
              style={[bulkAnimatedStyle, { marginTop: 12, marginBottom: 28 }]}
            >
              <Pressable
                onPressIn={() => {
                  bulkScale.value = withSpring(0.96);
                }}
                onPressOut={() => {
                  bulkScale.value = withSpring(1);
                }}
        ...
        >
          ...
          <Text
            style={[styles.bodyText, { color: isDark ? C.darkSub : "#374151" }]}
          >
            {product.aboutBody}
          </Text>
        </Animated.View>
        ...
        {product.usedFor && (
          <Animated.View
            ...
            style={[
              styles.section,
              { backgroundColor: isDark ? C.darkBg : C.cream },
            ]}
          >
            <Text style={[styles.eyebrow, { color: C.gold }]}>
              BEST USED FOR
              ...
              {product.usedFor.map((u, i) => (
                <View
                  ...
                  style={[
                    styles.usedCard,
                    {
                      backgroundColor: isDark ? C.darkCard : C.bg,
                      borderColor,
                    },
                  ]}
                >
                  <Text style={[styles.usedTitle, { color: textColor }]}>
                    {u.title}
                  </Text>
                  <Text style={[styles.usedDesc, { color: C.secondaryBlue }]}>
                    {u.desc}
                  </Text>
                </View>
              ))}
        ...
        {product.specs && (
          <Animated.View
            ...
            style={[styles.specsSection, { backgroundColor: C.primaryBlue }]}
          >
        ...
        {product.regulatory && (
          <Animated.View
            ...
            style={[
              styles.specsSection,
              { backgroundColor: C.primaryBlue, paddingTop: 0 },
            ]}
          >
            <Text style={[styles.specsEyebrow, { paddingTop: 28 }]}>
              REGULATORY STATUS
            </Text>
        ...
        >
          ...
          <Text style={[styles.orderDesc, { color: subColor }]}>
            {product.orderDesc}
          </Text>
          <Pressable
            onPress={handleAddToCart}
            style={[
              styles.cartBtn,
              { marginTop: 20, backgroundColor: C.primaryBlue, width: "100%" },
            ]}
          >
            <Ionicons
              name="cart"
            ...
            <View style={{ gap: 10, marginTop: 16 }}>
              {relatedProducts.map((rp) => (
                <RelatedCard
                  key={rp.id}
                  item={rp}
                  onPress={() => router.replace(`/our-products/${rp.id}`)}
                />
              ))}
            </View>
          </Animated.View>
...
const styles = StyleSheet.create({
  ...
  relatedSizeBox: {
    ...
    height: 52,
    borderRadius: 12,
    backgroundColor: C.primaryBlue + "15",
    alignItems: "center",
    justifyContent: "center",
  },
