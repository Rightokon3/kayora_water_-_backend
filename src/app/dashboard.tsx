...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    ...
    // Wait 400ms after user finishes typing before hitting the backend database
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: colors.white }]} edges={["top"]}>
      <Animated.View style={[styles.header, headerStyle]}>
        <View style={styles.headerLeft}>
          <Text style={[styles.logoText, { color: colors.primaryBlue }]}>KAYORA</Text>
          <Text style={[styles.headerSubtitle, { color: colors.grayText }]}>Our Products</Text>
        </View>
        ...
        <View style={styles.headerRight}>
          ...
          <Pressable onPress={handleProfilePress} style={styles.avatarButton} accessibilityLabel="Open settings">
            {profile && profile.profileImageUri ? (
              <Animated.Image source={{ uri: profile.profileImageUri }} style={styles.avatarImage} />
            ) : (
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyStateText, { color: colors.grayText }]}>
                {searchQuery ? `No products match "${searchQuery}"` : "No products available in database."}
              </Text>
            </View>
          }
...
const ProductCard = React.memo(function ProductCard({ product, index, columns, colors, onPress }: ProductCardProps) {
  ...
  useEffect(() => {
    ...
    translateY.value = withDelay(delay, withTiming(0, { duration: 360, easing }));
  }, [index]);
    ...
    <Animated.View style={[styles.cardOuter, cardWidthStyle, cardStyle]}>
      ...
      >
        <View style={styles.cardImageWrapper}>
          {product.image ? (
            <Animated.Image source={{ uri: product.image }} style={styles.cardImage} />
          ) : (
            <LinearGradient
...
const styles = StyleSheet.create({
  ...
  cardBody: { padding: 14 },
  cardSize: { fontSize: 11, fontWeight: "700", letterSpacing: 1, marginBottom: 4 },
  cardName: { fontSize: 18, fontWeight: "800", marginBottom: 2 },
  cardTagline: { fontSize: 12, fontWeight: "600", fontStyle: "italic", marginBottom: 8 },
  ...
  tabBar: { flexDirection: "row", borderTopWidth: 1, paddingTop: 8, paddingBottom: 10 },
